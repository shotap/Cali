...
module Cali {
    ...
    class EventList {
        ...
        static cloneEvent(_event:iEvent):iEvent {
            ...
            for (var attr in _event) {
                if (_event.hasOwnProperty(attr)) {
                    if (_event[attr] instanceof Date) {
                        copy[attr] = new Date(_event[attr]);
                    } else {
                        copy[attr] = _event[attr];
                    }
                }
        ...
        static appendEventElementToParent(el:Element, parent:Element) {
            ...
            this.buttons.forEach((view) => {
                switch (view) {
                    ...
                    case 'week':
                        ...
                        break;
                    case 'month':
                        this.addButton('month');
                        break;
                    case 'year':
                        this.addButton('year');
                        break;
                    case 'today':
                        this.addButton('today');
                        break;
                    case 'list':
                        this.addButton('list');
                        break;
    ...
    abstract class CaliContentView extends CaliView {
        ...
        getEventElement(event:iEvent, parent:Element, maxOffset:number, isWeek?:boolean, isMonth?:boolean):Element[] {
            ...
            if (isMonth) {
                ...
                if (event.start.getDate() === event.end.getDate()) { // start and end in the same day
    ...
    class CaliContentMonthView extends CaliContentView {
        ...
        renderGrid():void {
            ...
            for (let i = 0; i < 6; i++) {
                ...
                for (let j = 0; j < 7; j++) {
                    ...
                    if (cellDate.getDate() === 1) isActive = !isActive;

                    let tblRowCell = document.createElement('td');
                    tblRowCell.setAttribute('data-' + config.classPrefix + '-day-in-month', '');
                    if (!isActive) {
                        tblRowCell.setAttribute('data-' + config.classPrefix + '-day-in-month-not-active', '');
                    }
        ...
        renderContent():void {
            ...
            filttered[0].forEach((event, i) => this.getEventElement(event, list, filttered[1], false, true).forEach((e) => EventList.appendEventElementToParent(e, list)));
    ...
    class CaliContentWeekView extends CaliContentView {
        ...
        renderContent():void {
            ...
            let filttered:[iEvent[], number] = this.getFilteredEvents();
            filttered[0].forEach((event, i) => this.getEventElement(event, list, filttered[1], true).forEach((e) => EventList.appendEventElementToParent(e, list)));
            this.element.appendChild(list);
        }
    }
    class CaliContentDayView extends CaliContentView {
        ...
        renderContent():void {
            ...
            filtered[0].forEach((event, i) => this.getEventElement(event, list, filtered[1]).forEach((e) => EventList.appendEventElementToParent(e, list)));
            this.element.appendChild(list);
        }
    }
    ...
    class Cali {
        ...
        constructor(element:Element, events?:iEvent[]) {
            console.log(events);
            if (!element)
                throw 'You cant init cali withot an element';
            this.element = element;
        ...
        setView(type:string, toRender:boolean):void {
            ...
            switch (type) {
                case 'month':
                    contentObj = new CaliContentMonthView(this.contentElement, null);
                    break;
                case 'week':
                    contentObj = new CaliContentWeekView(this.contentElement, null);
                    break;
                case 'day':
                    contentObj = new CaliContentDayView(this.contentElement, null);
                    break;
                default:
                    throw 'Not supported view type';
            }
            if (contentObj) {
        ...
        getActiveDate():Date {
            return new Date(this.activeDate.toDateString());
        }
        ...
        next():void {
            switch (this.content.getViewName()) {
                case 'month':
                    this.activeDate.setFullYear(this.activeDate.getFullYear(), this.activeDate.getMonth() + 1);
                    break;
                case 'week':
                ...
                case 'day':
                    ...
                    break;
                case 'year':
                    this.activeDate.setDate(this.activeDate.getDate() + 365);
                    break;
            }
            this.render();
        }
    ...
    let calis:Cali[] = [];
    if (config.mobileResponsiveActive) {
        document.addEventListener('animationend', (event:AnimationEvent) => {
            switch (event.animationName) {
                case 'mobile-on':
                    calis.forEach((cali) => cali.setView(config.mobileResponsiveViewType, true));
                    break;
                case 'mobile-off':
                    calis.forEach((cali) => cali.setView(cali.contentElement.getAttribute('data-' + config.classPrefix + '-content'), true));
                    break;
            }
        }, false);
    }
    ...
    for (let i = 0; i < caliElements.length; i++) {
        calis.push(new Cali(caliElements[i], [
            {uid: 0, title:'a', start: new Date('2017/05/07 03:00:00'), end: new Date('2017/05/07 06:21:00')},
            {uid: 1, title:'b', start: new Date('2017/05/08 04:00:00'), end: new Date('2017/05/08 06:21:00')},
            {uid: 2, title:'c', start: new Date('2017/05/09 05:00:00'), end: new Date('2017/05/10 06:21:00')},
            {uid: 3, title:'d', start: new Date('2017/05/09 06:00:00'), end: new Date('2017/05/09 06:21:00')},
            {uid: 4, title:'e', start: new Date('2017/05/10 07:00:00'), end: new Date('2017/05/11 06:21:00')}
        ]));
    }
}